...
app.get('/shippings', cors(), (req, res) => {
    var queryStr = "SELECT * FROM shippings WHERE 1";
...
app.get('/shippings/:id', cors(), (req, res) => {
    const item = req.params.id;
    //console.log("is number: " + !isNaN(item));
    if (item) {
        console.log("search key: " + item);
        var queryStr = "SELECT * FROM shippings WHERE shippings.order_id LIKE '" + item + "%' OR shippings.is_complete LIKE '" + item + "%' OR shippings.address LIKE '" + item + "%'";
        con.query(queryStr, function (error, result, fields) {
            if (error) {
                console.log(error);
...
app.post('/shippings', cors(), (req, res) => {
    const obj = req.body;
    console.log(obj);
    var queryStr = "INSERT INTO shippings (order_id, is_complete, address) VALUES ('" + obj.order_id + "', '" + obj.is_complete + "', '" + obj.address + "')";
    con.query(queryStr, function (error, result, fields) {
        if (error) {
            console.log(error);
            res.status(400);
        ...
        if (result) {
            ...
            res.status(201);
            res.send("location: /shippings/" + result.insertId);
        }
    });
});


app.delete('/shippings/:id', cors(), (req, res) => {
    const shipping_id = req.params.id;
    var queryStr = "DELETE FROM shippings WHERE shippings.shipping_id = " + shipping_id;
    con.query(queryStr, function (error, result, fields) {
        if (error) {
            console.log(error);
            res.status(400);
...
app.put('/shippings/:id', cors(), (req, res) => {
    const obj = req.body;
    const shipping_id = req.params.id;
    console.log(obj);
    var queryStr = "UPDATE shippings SET order_id = '" + obj.order_id + "', address = '" + obj.address + "', is_complete = '" + obj.is_complete + "' WHERE shipping_id='" + shipping_id + "'";
    con.query(queryStr, function (error, result, fields) {
        if (error) {
            console.log(error);
            res.status(400);
